...
async function fillAndSearchWithDouban(title) {
    ...
    if (typeof window.ensureSearchableApisSelected === 'function') {
        window.ensureSearchableApisSelected();
    }

    if (typeof selectedAPIs !== 'undefined' && selectedAPIs.length === 0) {
        showToast('当前没有可用的数据源，请先在设置中选择或添加API', 'warning');
        return;
    }
...
async function loadDoubanProxyImage(img) {
    if (!img) return;
    ...
    const encodedCover = img.dataset.encodedCover;
    if (!encodedCover) {
        img.onerror = null;
        img.src = 'image/nomedia.png';
        img.classList.add('object-contain');
    ...
    let proxyUrl = `${PROXY_URL}${encodedCover}`;
